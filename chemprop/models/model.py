...
class CMPNAdapter(nn.Module):
    ...
    def __init__(self, cmpn_encoder):
        ...
        self.encoder = cmpn_encoder.encoder  # 暴露 encoder 供 add_functional_prompt 使用
                    ...
                    if fg_states.size(0) != num_atoms:
                        if fg_states.size(0) > num_atoms:
                            ...
                            if not getattr(self, '_warned_missing_kg', False):
                                print(
                                    "⚠️ [Warning] KAPT: ElementKG features missing in Combine Mode. Falling back to noise.")
