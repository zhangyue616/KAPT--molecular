...
def add_train_args(parser: ArgumentParser):
    ...
    parser.add_argument('--use_kapt', action='store_true', default=False,
                        help='Enable KAPT (Knowledge-Aware Prompt Tuning). Defaults to Task-Only mode if --combine is not set.')
    parser.add_argument('--combine', action='store_true', default=False,
                        help='Enable Full Synergy mode (Backbone + KG + Task). Overrides --use_kapt.')
    ...
    parser.add_argument('--freeze_backbone', action='store_true', default=False,
                        help='Freeze the backbone model')
...
def modify_train_args(args: Namespace):
    ...
    if args.combine:
        ...
        print("\n🚀 [Config] Activated: Full Synergy (Backbone + KG + Task)\n")
    elif args.use_kapt:
        args.step = 'pretrain'
        ...
        args.use_kapt = False
        args.step = 'functional_prompt'
        print("\n🚀 [Config] Activated: Baseline Mode\n")

    if hasattr(args, 'structure_noise_scale'):
        args.noise_scale = args.structure_noise_scale

    if not args.freeze_backbone and args.backbone_lr >= args.prompt_lr and args.use_kapt:
        import warnings
        warnings.warn(f'Warning: backbone_lr ({args.backbone_lr}) >= prompt_lr ({args.prompt_lr})')
