"""
KAPT Training Script

Supports two modes:
1. Baseline mode: python train.py --step functional_prompt ...
2. KAPT mode: python train.py --use_kapt --step kapt ...
"""
import random
import sys
import logging
...
def setup_logging(args):
    logger = logging.getLogger('chemprop-kapt')
    ...
    mode_tag = 'KAPT' if args.use_kapt else 'Baseline'
    formatter = logging.Formatter(
        f'%(asctime)s [{mode_tag}] %(levelname)s - %(message)s',
        datefmt='%Y-%m-%d %H:%M:%S'
    )
...
def print_experiment_info(args, logger):
    ...
    logger.info(f'Mode: {"KAPT" if args.use_kapt else "Baseline"}')
...
def validate_kapt_requirements(args, logger):
    ...
    try:
        ...
        logger.error('Please ensure chemprop/models/kapt_modules.py exists.')
        return False
